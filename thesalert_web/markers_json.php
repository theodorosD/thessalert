...
$pdo=new PDO("mysql:dbname=$dbname;host=$host",$username,$password,array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8",  PDO::ATTR_PERSISTENT => true));
...
$statement=$pdo->prepare("SELECT address, description, alerts.id, lng, lat, time, image, title,alert_types.alert_name as alertname, alert_types.id AS alerttypeid,alert_types.alert_icon
FROM alerts LEFT JOIN alert_types ON alerts.alerttype = alert_types.id WHERE showhide=TRUE AND time BETWEEN (SYSDATE() - INTERVAL 1 DAY) AND SYSDATE()");
}
