...
if ($_FILES['photo']['size'] > ($maxsize * 1048576)) {
    ...
    $r->error = "Λάθος: Μέγιστο μέγεθος εικόνας: $maxsize Kb";
}

// If the file is an image
if (preg_match('/image/i', $_FILES['photo']['type'])) {
    $uniq=uniqid();
    $filename = './assets/alertimages/' . $uniq . '.jpg';
    $thumb_filename = './assets/alertimages/thumb_' . $uniq . '.jpg';
     
} else {

    $r->error = "Λάθος: Μη αποδεκτός τύπος αρχείου.";
}
...
if (in_array($_FILES['photo']['type'], $types)) {
    ...
    $r->error = "Λάθος: Αυτό το αρχείο δεν είναι εικόνα.";
    return false;
}

//Create thumbnail and resize our image
if (file_exists($filename)) {
if(copy($filename, $thumb_filename)){
exec("convert $thumb_filename -resize 320x150^ -gravity Center -crop 320x150+0+0 +repage $thumb_filename");
exec("convert $filename -resize 640x640\> $filename");
}
}
